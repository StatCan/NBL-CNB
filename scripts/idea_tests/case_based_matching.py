...
# Iterate over the different case types
for lyr_nme in fiona.listlayers(ap_cases_gpkg):
    
    if (lyr_nme == 'ap_no_p_link') or ('_any_' in lyr_nme): continue # only test layers with parcel matches for now

    print(f'Running matching on cases: {lyr_nme}')

    addresses = gpd.read_file(ap_cases_gpkg, layer=lyr_nme, crs=26911)
    ...
    merge = addresses.merge(footprint[~footprint[join_footprint].isna()][[join_footprint, "footprint_index"]], how="left", left_on=join_addresses, right_on=join_footprint)
    ...
    if len(unlinked_aps) > 0:
        ...
        print(f'     processing {len(unlinked_aps)} unlinked geometries')
    ...
    outgdf.to_file(output_gpkg, layer=f'{lyr_nme}_bf_links',  driver='GPKG')

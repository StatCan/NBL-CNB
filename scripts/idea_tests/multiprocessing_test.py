...
def main():
    ...
    def get_unlinked_geometry(addresses_gdf, footprint_gdf , buffer_distance):
        '''Returns indexes for the bf based on the buffer'''
        ...
        def list_bf_indexes(buffer_geom, bf_gdf):
            ...
            if len(intersects) > 0:
                ...
                return np.nan
        print(len(addresses_gdf))
        addresses_gdf['buffer_geom'] = addresses_gdf.geometry.buffer(buffer_distance)
        addresses_gdf['footprint_index'] = addresses_gdf['buffer_geom'].swifter.apply(lambda point_buffer: list_bf_indexes(point_buffer, footprint_gdf))
    ...
    print('     running bp case extraction')
    ...
    print('     getting unlinked geometry bp cases')
    addresses_bp = get_unlinked_geometry(addresses_bp, footprint, 20)
    return
    # Find and reduce plural linkages to the closest linkage
    print('     find and reduce plural linkages to the closest linkage (bp cases)')

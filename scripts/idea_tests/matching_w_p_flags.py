...
# Grab all the many to many's and many to one's for the flag checking process that have a record that matches the parcel location field. 
addresses_many = addresses.loc[((addresses['parcel_rel'] == 'many_to_many') | (addresses['parcel_rel'] == 'many_to_one')) & (addresses['parcel_location_match'] == 0)]
addresses = addresses[~addresses['link_field'].isin(addresses_many.index.tolist())]
# Convert linkages to integer tuples, if possible.
addresses_many["footprint_index"] = addresses_many["footprint_index"].map(lambda vals: tuple(set(map(as_int, vals))))

# Flag plural linkages.
flag_plural = addresses_many["footprint_index"].map(len) > 1
addresses_many = addresses_many.explode('footprint_index') # Convert the lists into unique rows per building linkage (cleaned up later)

addresses_many = addresses_many[addresses_many['footprint_index'] != np.nan]
addresses_many.method.fillna('flagged_data_linking', inplace=True)
footprint['centroid_geo'] = footprint['geometry'].apply(lambda bf: bf.centroid)
addresses_many['out_geom'] = addresses_many['footprint_index'].apply(lambda row: create_centroid_match(row, footprint['centroid_geo']))

addresses_many = addresses_many.set_geometry('out_geom')

addresses_many.drop(columns='geometry', inplace=True)
addresses_many.rename(columns={'out_geom':'geometry'}, inplace=True)
addresses_many = addresses_many.set_geometry('geometry')

addresses_many.to_file(r'C:\projects\point_in_polygon\data\NB_data\parcelflag_filter.gpkg', layer='linkage_filter_test',  driver='GPKG')

print(addresses_many)
print(len(addresses_many))
print(addresses_many.columns)
sys.exit()

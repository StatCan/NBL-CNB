...
2. Polygons that intersect a line are then split along the lines that they intersect and a new MultiPolygon object is returned. To accomplish this the following steps 
   ...
   a. Create a spatial join between the building polygons and the cleaned line geometry. 
   ...
   b. For each building polygon if it intersects any lines do the following:
   ...
   c. Take the output from the previous step and explode it so that every individual Polygon object
      is in its own record. Delete all non essential fields.
